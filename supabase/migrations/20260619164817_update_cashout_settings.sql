...
begin
  ...
  -- Basic VIP: 150 minutes. Premium VIP: 500 minutes. Renewal: 200 minutes.
  ...
  -- Check for Streak bonus (3 in 7 days): +1000 min bonus
  ...
  if v_subs_since_last_streak >= 3 then
    ...
    if not exists (
      ...
      -- Increment member_minutes.gifted_minutes for the female by another 1000
      update public.member_minutes
      set gifted_minutes = coalesce(gifted_minutes, 0) + 1000,
          updated_at = now()
      where user_id = v_female_id;
    end if;
  end if;
...
begin
  ...
  -- Load the current cashout rate, falling back to 0.01 if missing
  ...
  v_rate_per_minute := coalesce(v_rate_per_minute, 0.01);
