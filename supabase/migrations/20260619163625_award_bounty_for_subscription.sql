...
BEGIN
  ...
  -- Basic VIP: 300 minutes. Premium VIP: 1000 minutes. Renewal: 400 minutes.
  IF v_source = 'renewal' THEN
    v_amount_minutes := 400;
  ELSIF v_source = 'premium' THEN
    v_amount_minutes := 1000;
  ELSE
    v_amount_minutes := 300;
  END IF;
  ...
  -- Check for Streak bonus (3 in 7 days): +2000 min bonus
  ...
  IF v_subs_since_last_streak >= 3 THEN
    ...
    IF NOT EXISTS (
      ...
      -- Increment member_minutes.gifted_minutes for the female by another 2000
      UPDATE public.member_minutes
      SET gifted_minutes = COALESCE(gifted_minutes, 0) + 2000,
          updated_at = now()
      WHERE user_id = v_female_id;
    END IF;
  END IF;
