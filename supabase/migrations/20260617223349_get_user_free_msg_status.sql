...
DECLARE
  ...
  v_result jsonb;
BEGIN
  -- Match the frontend hook: VIP is based on is_vip only
  SELECT COALESCE(mm.is_vip, false)
  INTO v_is_vip
  FROM public.member_minutes mm
  WHERE mm.user_id = target_user_id;

  v_is_vip := COALESCE(v_is_vip, false);

  -- Count all messages sent by the target user to female members
  ...
  WHERE m.sender_id = target_user_id
    AND lower(COALESCE(partner.gender, '')) = 'female';
