...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`[google-bounty-webhook] user=${user.id} sku=${sku} purchaseToken=${purchaseToken}`);
    ...
    const idempotencyKey = `iap:google:${purchaseToken}`;
    ...
    if (iapError) {
      console.warn("[google-bounty-webhook] Error inserting into iap_purchases:", iapError);
      // Continue anyway
    }
    ...
    if (upsertError) {
      console.error("[google-bounty-webhook] Error updating member_minutes:", upsertError);
      throw upsertError;
    }
    ...
    try {
      ...
      if (refreshError) {
        console.warn("[google-bounty-webhook] Failed to fetch updated member_minutes:", refreshError);
      } else {
        memberMinutes = refreshedMinutes ?? null;
      }
    } catch (refreshError) {
      console.warn("[google-bounty-webhook] Failed to read updated member_minutes:", refreshError);
    }

    // 3. Award bounty before responding to ensure notifications are sent
    ...
    try {
      ...
      if (member?.gender?.toLowerCase() === "male") {
        console.log(`[google-bounty-webhook] User is male. Invoking bounty RPC...`);
        ...
        if (rpcError) {
          console.warn("[google-bounty-webhook] award_bounty_for_subscription returned error:", rpcError);
        } else {
          ...
          console.log("[google-bounty-webhook] award_bounty_for_subscription result:", result);
          
          if (bountyAwarded) {
            console.log(`[google-bounty-webhook] Triggering bounty notification for female: ${result.female_id}`);
            ...
            if (notifyError) {
              console.error("[google-bounty-webhook] Failed to trigger notify-bounty:", notifyError);
            } else {
              console.log("[google-bounty-webhook] notify-bounty triggered successfully:", notifyData);
            }
          }
        }
      } else {
        console.log(`[google-bounty-webhook] User gender is not male (${member?.gender}), skipping bounty award.`);
      }
    } catch (bountyError) {
      console.error("[google-bounty-webhook] Failed to award bounty:", bountyError);
    }
    ...
    console.error("[google-bounty-webhook] error:", error);
