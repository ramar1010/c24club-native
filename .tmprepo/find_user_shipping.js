...
async function run() {
  const select = 'id,email,shipping_name,shipping_address,shipping_city,shipping_state,shipping_zip,shipping_country';
  const filter = `email=eq.${encodeURIComponent(email)}`;
  const results = await query('members', select, filter);
