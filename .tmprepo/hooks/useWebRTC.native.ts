/**
 * useWebRTC.native.ts — Native WebRTC hook (matches web app signaling exactly)
 * 
 * Signaling: room_signals table with polling every 1000ms
 * Fields: room_id, sender_channel, signal_type, payload
 * Offer sender: the poller (member who was waiting in queue)
 * Answerer: the member who was matched immediately (partner_found)
 */
...
// ─── Platform guard ───────────────────────────────────────────────────────────
export const isWebRTCAvailable: boolean = Platform.OS !== 'web';
...
// ─── DB Signaling helpers (matches web app field names exactly) ───────────────
async function dbSendSignal(roomId: string, senderChannel: string, signalType: string, payload: any) {
  try {
    ...
    console.warn('[WebRTC] dbSendSignal error:', e);
  }
}
  ...
  const startSignalPolling = useCallback((roomId: string) => {
    ...
    signalPollIntervalRef.current = setInterval(async () => {
      ...
      try {
        ...
        console.warn('[WebRTC] Signal poll error:', e);
      }
    }, 1000);
